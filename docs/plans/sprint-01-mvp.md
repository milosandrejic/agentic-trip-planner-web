# Sprint 1 — MVP (archived)

> **Archived.** This sprint is closed — every phase below is done. The current sprint lives
> at `docs/PLAN.md`; durable rules that outlived this sprint moved to `docs/PRODUCT_RULES.md`
> and `docs/CODING_STYLE.md`. Kept for history; nothing here is a live instruction.

> Built a production-grade Next.js (App Router) frontend for the AI Travel Planner, matching
> the exported Figma screens in `/design` and `docs/CODING_STYLE.md`. The API layer was wired
> strictly against `docs/API.md` — no invented endpoints. `docs/API.md` was regenerated from
> the live OpenAPI schema on 2026-08-31; Phase 9 migrated the frontend onto it.
...
Governed this sprint; still governs. Current copy: `docs/PLAN.md`.
...
- Each task lists: **Goal**, **Files**, **Depends**, **Commit**.
...
- [x] **1.2 Lint / format / editor config**
  ...
  - **Outcome:** Prettier was not added — no dependency, no `.prettierrc`/`.prettierignore`.
    ESLint (`@stylistic`) owns formatting instead; `.editorconfig` shipped as planned.
...
- [x] **9.3 Activity type → icons and map pins**
  ...
  - **Outcome:** The `activity_type` → icon/colour/pin mapping shipped in
    `src/constants/activity-types.ts`, not `activity-display.ts`; that file now holds only
    `formatDuration`. `map-markers.ts` and `activity-item.tsx` are as planned.
...
- [x] **9.10 Clarification rounds, 409 and the long turn**
  - **Goal:** The planner asks before it plans, and a partial answer asks again — so the
    ...
    ≤120 s wait a real progress affordance (see `docs/PLAN.md` 10.4, "Motion and the planning
    wait" — renumbered after this task shipped).
